...
We are building an automated weekly “pulse” that turns public Google Play Store reviews for the Groww fintech product into a one-page insight report and delivers it to stakeholders through Google Workspace. We will create and provide a custom MCP (Model Context Protocol) server within this project so that writes to Google Docs and Gmail go through this dedicated MCP server—not ad hoc API calls inside the agent.
...
- Ingest public reviews from the last 8–12 weeks (configurable window) from Google Play (scraper-based), specifically for Groww.
...
- Deliver outputs through the custom Google Workspace MCP server built in this project:
...
| Human-visible delivery | Custom MCP server (provided in this project) → Google Docs + Gmail |

The agent is an MCP host/client; it does not embed Google credentials or call the Docs/Gmail REST APIs directly for delivery. The custom MCP server will handle these interactions.

## Key requirements
- **MCP-based delivery:** Append to the shared Google Doc and send Gmail only via the provided custom MCP server’s tools (e.g. document batch update, draft/create/send flows as defined in architecture).
...
- Social sources (Twitter, Reddit, etc.) or other app stores (Apple App Store) in the initial scope.
...
- Each run adds one clearly labeled section to the pulse Google Doc (dated / week-labeled).
